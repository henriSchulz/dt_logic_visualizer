 import { renderSymmetryDiagram } from './symmetry-diagram/ui.js';
    ...
    renderTruth();
    renderKMap();
    const sortedTruth = truth.slice().sort((a, b) => a.bits.localeCompare(b.bits));
    renderSymmetryDiagram(nVars, sortedTruth.map(t => t.out))
    ...
    setTimeout(() => {
        document.querySelectorAll('#symmetry-diagram div[data-bits]').forEach(td=>{
        td.onclick=e=>{
            const currentTarget = e.currentTarget;
            ...
            const currentValueIndex=truth.findIndex(t=>t.bits===bits);
            
            if (currentValueIndex === -1) return; // No matching bits found

            const currentValue = truth[currentValueIndex];
            
            const nextValue = currentValue.out === 0 ? 1 : currentValue.out === 1 ? null : 0;

            truth[currentValueIndex] = {
                out: nextValue,
    ...
    arr.forEach(b=>{
        ...
        document.querySelectorAll(`.card-body [data-bits="${b}"]`)
                .forEach(n=>n.classList.toggle('hl-cell',on)); 
        // Highlight cells in symmetry diagram (not inside card-body)
        document.querySelectorAll(`#symmetry-diagram [data-bits="${b}"]`)
    ...
    function init() {
        ...
        function adjustMuxCardHeight() {
            ...
            if (truthTableCard && muxCard && muxWrap) {
                ...
                const isTruthTableVisible = window.getComputedStyle(truthTableCard).display !== 'none';
